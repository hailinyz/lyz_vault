缓存穿透
![](assets/Redis/file-20260427183826586.png)
![](assets/Redis/file-20260427200346616.png)
总结
![](assets/Redis/file-20260427200530142.png)
缓存击穿
![](assets/Redis/file-20260427200707946.png)
![](assets/Redis/file-20260427201141003.png)
总结
![](assets/Redis/file-20260427201157265.png)
缓存雪崩
![](assets/Redis/file-20260427201428470.png)
总结
![](assets/Redis/file-20260427201606933.png)
![](assets/Redis/file-20260427201817709.png)


Redis作为缓存，MySQL的数据如何与Redis进行同步呢（双写一致性）
![](assets/Redis/file-20260427202020586.png)
双写一致：当修改了MySQL的数据也要同时更新Redis的数据，缓存和数据库的数据**保持一致**
![](assets/Redis/file-20260427202242634.png)
读操作：缓存命中，直接返回；缓存未命中查询数据库，写入缓存，设定超时时间
写操作：延时双删
![](assets/Redis/file-20260427202412217.png)
1.先删除缓存。还是先修改数据库
答案是，这两种都有可能出现脏数据
![](assets/Redis/file-20260427202824468.png)
为什么要删除啊两次缓存
为什么要延时双删：就是为了**降低脏数据**的出现；因为数据库是主从模式（读写分离），要等主节点同步到从节点（所以需要延时一会），所以延时双删极大控制了**有脏数据的风险**，因为这个延时的时间不好控制，所以还是**做不到绝对的强一致**。

强一致性的解决方案：
1. 那怎么才能做到**双写一致（强一致）**：可以用**分布式锁**
![](assets/Redis/file-20260427203947957.png)

但是性能低
可以优化：
一般放入缓存中的数据都是**读多写少**的 ，写多读少的直接操作数据库多省事啊
可以用**读写锁（读锁、排他锁）**
![](assets/Redis/file-20260427204720583.png)

2. 异步通知保证数据的最终一致性
![](assets/Redis/file-20260427205943144.png)
基于Canal的异步通知（接受短暂延时可接受）
![](assets/Redis/file-20260427210030125.png)

总结
![](assets/Redis/file-20260427210244493.png)

Redis持久化
1. RDB（数据备份文件）
![](assets/Redis/file-20260429125519031.png)
RDB执行原理
![](assets/Redis/file-20260429185500608.png)


2. AOF（追加文件）
![](assets/Redis/file-20260429185924000.png)
![](assets/Redis/file-20260429190009967.png)
![](assets/Redis/file-20260429190813741.png)

Redis数据过期策略
+ 惰性删除
当key过期后，不管它，需要用该key时，再检查是否过期。
如果过期，就删掉它，反之返回该key
![](assets/Redis/file-20260429210250163.png)
**优点**：对CPU友好，不浪费时间进行过期检查
**缺点**：对内存不好，如果一直没用，该key会一直存在内存中，用不释放

+ 定期删除
![](assets/Redis/file-20260429210817175.png)


Redis的**数据淘汰策略**，有8种
![](assets/Redis/file-20260429211347885.png)
**理解LRU：最近最少使用（删掉离现在时间更长的key）**
**理解LFU：最少频率使用（删掉使用次数最少的key）**


Redis分布式锁使用场景：集群情况下的**定时任务、抢单、幂等性**场景
![](assets/Redis/file-20260429220710597.png)
在**单体**项目中可以加这样的**互斥锁**防止**超卖**

但是我们的项目为了能够更多的**并发请求**，会把服务做成**集群**部署
![](assets/Redis/file-20260429220844810.png)
synchronized锁是属于**本地的锁**，这个锁是属于JVM的，那么每个服务都有各自的JVM，它只能解决统一个JVM下**线程的互斥**，解决不了**多个JVM下线程的互斥**
![](assets/Redis/file-20260429221235446.png)

所以**在集群的情况下**，我们就**不能使用本地的锁来解决**

需要用**外部的锁------分布式锁**解决
![](assets/Redis/file-20260429221403871.png)
所以**项目有秒杀的业务、并且部署了集群------使用分布式锁防止超卖**（服务之间线程的互斥性）


redisson实现的分布式锁
线程id，可重入，redis的分布式锁不可重入
![](assets/Redis/file-20260429222738843.png)
![](assets/Redis/file-20260429223108166.png)


Redis其他的面试问题（高频）
