...
**好的，到这里mysql所有数据库和其他依赖组件的数据库已经创建**接下来可以部署nacos到服务器
第二版
```yml
oj-nacos-server:  
...
nacos.core.auth.server.identity.key=${NACOS_AUTH_IDENTITY_KEY:BITOJ202403031025}  
nacos.core.auth.server.identity.value=${NACOS_AUTH_IDENTITY_VALUE:BITOJ202403031025}  
...
```
要记住把这个nacos文件夹也要上传到服务器，不然**挂载失败导致部署nacos失败**
![](assets/项目部署（本地虚拟机）/file-20260327112128446.png)
部署完成nacos之后要对nacos**进行配置**
## Nacos配置
首先登录到nacos，浏览器输入 ：host:8848/nacos
初始账号密码：nacos
记得登陆上后改密码

然后修改各个微服务的yml文件（换成服务器ip、增加鉴权）
![](assets/项目部署（本地虚拟机）/file-20260327161208907.png)

修改好这些后，**就可以把nacos配置填入nacos上了**

oj-system-local.yaml
```yaml
server:
  port: 9201
...
spring:
  data:
    redis:
      ...
      password: 123456
  datasource:
    url: jdbc:mysql://192.168.100.131:3306/bitoj_dev?useUnicode=true&characterEncoding=utf8&useSSL=true&serverTimezone=GMT%2B8
    username: ojtest
    password: 123456
  elasticsearch:
...
jwt:
  secret: zxcvbnmasdfghjuiyreqtuiwq
```
oj-gateway-local.yaml
```yaml
server:
  port: 19090
...
sms:
  ...
  is-send: false
  aliyun:
    accessKeyId: 你的实际创建的数据
    accessKeySecret: 你的实际创建的数据
    endpoint: 你的实际创建的数据
    templateCode: 你的实际创建的数据
    sing-name: 你的实际创建的数据
...
file:
  ...
  oss:
    ...
    accessKeyId: 你的实际创建的数据
    accessKeySecret: 你的实际创建的数据
    bucketName: 你的实际创建的数据
    pathPrefix: 你的实际创建的数据
    downloadUrl: 你的实际创建的数据
```
oj-job-local.yaml
```yaml
server:
...
spring:
  datasource:
    url: jdbc:mysql://192.168.100.131:3306/bitoj_dev?useUnicode=true&characterEncoding=utf8&zeroDateTimeBehavior=convertToNull&useSSL=true&serverTimezone=GMT%2B8&allowMultiQueries=true
    username: ojtest
    password: 123456
  data:
    redis:
...
xxl:
  job:
    admin:
      addresses: http://192.168.100.131:8080/xxl-job-admin
...
完成nacos之后就可以进行第三版的docker compose 部署
## 第三版docker compose部署
```yml
...
services:  
  oj-mysql-server:  
    ...
    command:  
      - --character-set-server=utf8mb4  
      - --collation-server=utf8mb4_general_ci  
    # 通过执行特定的 MySQL 命令来检查服务的健康状态 用于测试连接到 MySQL 服务器  
    healthcheck:  
      test: [ "CMD", "mysqladmin", "ping", "-u", "root", "-p123456" ]  
      interval: 10s  
      timeout: 5s  
...
```
执行脚本将每个微服务的jar包移动到对应的文件夹下
注意：打包好的jar包要更名为这样的形势![](assets/项目部署（本地虚拟机）/file-20260329085849786.png)

部署完成后验证：
C端：192.168.100.131:80
B端：192.168.100.131:10030

登录B端之后因为还没有管理员，要预制一个，使用之前预留的system里的add接口（apifox）调用一下就可以了（注意ip改成服务器的）
![](assets/项目部署（本地虚拟机）/file-20260329090238129.png)

在预制管理员的时候，因为system的add接口是走网关的，要先加入白名单，预制完成再从白名单中去掉就行。
![](assets/项目部署（本地虚拟机）/file-20260329090514030.png)

服务繁忙请稍后重试是因为当前没有用户，但是要求有create_by，因为我们从线程池里获取用户信息，所以会失败，可以直接从系统创建一个1L用户，然后再调用。
![](assets/项目部署（本地虚拟机）/file-20260329091330140.png)
然后重新打包上传启动就可以

**注意**：记得把token这个参数取消掉
![](assets/项目部署（本地虚拟机）/file-20260329100449341.png)
