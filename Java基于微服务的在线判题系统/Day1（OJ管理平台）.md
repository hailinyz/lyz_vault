...
## CS架构
Client客户端-Socket服务器
![](assets/Day1（OJ管理平台）/file-20260325095225945%201.png)
特点：
...
## BS架构
浏览器(browser)-服务器架构
![](assets/Day1（OJ管理平台）/file-20260325095225945.png)
特点：
...
| 定时任务服务   | 定时任务增删改查、定时任务功能实现                                            | 从技术的角度分析定时任务的执行可能需要和多个服务配合完成。并且定时任务的执行可能会周期性过长时间占用资源。那么我们可以将定时任务管理拆分出来。 |
...
![](assets/Day1（OJ管理平台）/file-20260325095225943.jpeg)
...
### B端
面向企业（Business）客户的软件，这类通常有复杂功能和高度定制化的特点，涵盖各个行业，财务、人力资源、供应链管理、企业资源规划（ERP）等

### C端
面向普通消费者(Consumer) 的软件，侧重于用户体验

咱们这个项目 B端有第三方的任务定时、管理员登陆的那个
C端有答题系统