...
题目列表：
先查ES，如果能查到 --> 前端
如果查不到，在查mysql。如果能查到 --> 数据同步到ES，并且将查到的数据返回前端
如果从数据库中也查不到，返回前端 空。
...
创建一个类似mysql操作数据库的mapper
```java
package com.bite.friend.elasticsearch;  
  
import com.bite.friend.controller.question.es.QuestionES;  
