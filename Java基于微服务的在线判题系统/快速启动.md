...
![](assets/快速启动/file-20260622135742357.png)
# 创建 `oj-network` 网桥 = **给你的 OJ 项目单独划分一块私有局域网**，所有组件在里面靠名字互相访问，隔离干净、配置稳定、部署省心。
```powershell
...
# 启动es容器
```powershell
docker run -d --name oj-es-dev -e "ES_JAVA_OPTS=-Xms256m -Xmx256m" -e "discovery.type=single-node" -v D:\Project\bite-oj\deploy\dev\elasticSearch\es-plugins:/usr/share/elasticsearch/plugins -e "xpack.security.enabled=false" --privileged --network oj-network -p 9200:9200 -p 9301:9300 elasticsearch:8.5.3
```
# 启动kibana
```powershell
...
spring:  
  application:  
    name: oj-system  # 每个服务的名字不一样，复制到这个服务后记得改
  profiles:  
    active: local  
  cloud:  
    nacos:  
...
spring:
  data:
    redis:
      host: 项目跑在哪个服务器就用哪个ip
      password: 123456
  datasource:
    url: jdbc:mysql://项目跑在哪个服务器就用哪个ip:3306/bitoj_dev?useUnicode=true&characterEncoding=utf8&useSSL=true&serverTimezone=GMT%2B8
    username: ojtest
    password: 123456
  elasticsearch:
    uris: http://项目跑在哪个服务器就用哪个ip:9200
...
spring:
  data:
    redis:
      ...
      password: 123456
  datasource:
    url: jdbc:mysql://项目跑在哪个服务器就用哪个ip/bitoj_dev?useUnicode=true&characterEncoding=utf8&useSSL=true&serverTimezone=GMT%2B8
    username: ojtest
    password: 123456
  elasticsearch:
    uris: http://项目跑在哪个服务器就用哪个ip:9200
...
spring:
  datasource:
    url: jdbc:mysql://项目跑在哪个服务器就用哪个ip:3306/bitoj_dev?useUnicode=true&characterEncoding=utf8&zeroDateTimeBehavior=convertToNull&useSSL=true&serverTimezone=GMT%2B8&allowMultiQueries=true
    username: ojtest
    password: 123456
  data:
    redis:
...
xxl:
  job:
    admin:
      addresses: http://项目跑在哪个服务器就用哪个ip:8080/xxl-job-admin
...
spring:
  datasource:
    url: jdbc:mysql://项目跑在哪个服务器就用哪个ip/bitoj_dev?useUnicode=true&characterEncoding=utf8&zeroDateTimeBehavior=convertToNull&useSSL=true&serverTimezone=GMT%2B8&allowMultiQueries=true
    username: ojtest
    password: 123456
  rabbitmq:
    host: 项目跑在哪个服务器就用哪个ip
...
sandbox:
  docker:
    host: tcp://这个是填入你的ipaddress:2375
  limit:
    memory: 100000000
...
# 注意事项
## 切面xxl-job
![](assets/快速启动/file-20260622191141177.png)
![](assets/快速启动/file-20260622191149102.png)
![](assets/快速启动/file-20260622191207519.png)
...
## judge服务nacos配置
你配置里 `sandbox.docker.host: tcp://172.17.0.2:2375`
`172.17.0.2` 是 Docker 容器内部 dind 容器的内网 IP，**Windows 本地宿主机根本访问不到**，所以连接超时报错。
所以改成
![](assets/快速启动/file-20260622205351191.png)
...
- 重启 oj-judge 服务即可
## 预制系统管理员
![](assets/快速启动/file-20260622211800652.png)
