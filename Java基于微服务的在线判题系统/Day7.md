...
竞赛编辑编辑 = 竞赛详情功能 + 竞赛基本信息编辑 + 竞赛题目信息编辑
...
1. 找到竞赛中要删除的题目，点击删除按钮，前端携带竞赛id  和 题目 id, 向后端发起删除竞赛中题目请求
2. 后端接收到请求之后，查出要删除的竞赛 ，并判断是否能进行操作
   如果不能操作，返回前端不能操作的原因
   如果能，就根据题目id 删除竞赛中题目，并将删除结果返回前端
3. 前端接收响应如果成功/失败
...
虽然说现在竞赛编辑接口到这里已经全部完成了，但是有个小问题：**竞赛中添加题目显示出来的题目列表和题目管理的题目列表用的是同一个接口**，导致竞赛中已经有的题目仍然会显示在列表中，可能对于用户体验不好
