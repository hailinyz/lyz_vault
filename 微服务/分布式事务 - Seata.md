...
![](assets/分布式事务%20-%20Seata/file-20251128164802518.png)
...
+ TM - 事务管理器
+ RM - 资源管理器
![](assets/分布式事务%20-%20Seata/file-20251128164802517.png)
...
```
我这里查出来的是在hmall下
![](assets/分布式事务%20-%20Seata/file-20251128164802502.png)

3. 再查以下nacos容器，我这里查出来是不在hmall里的
![](assets/分布式事务%20-%20Seata/file-20251128164802501.png)
如果不在就可以把已经存在的加进来：
```powershell
...
![](assets/分布式事务%20-%20Seata/file-20251128164802498%201.png)
...
原理：当微服务**执行sql**的时候还没有提交，会先**上锁**等待，等待TC检查事务状态之后才会**解锁**提交，
...
②RM接收TC指令，提交或回滚事务
![](assets/分布式事务%20-%20Seata/file-20251128164802498.png)
优点：确保了ACID的特性，强一致
缺点： **上锁**等待（双刃剑），有些业务是串行执行，每个分支都耗时，所以第一个分支久等了很久，就降低了性能。依赖于关系型数据库实现事务，如果有些数据库不支持XA模式，那就很尴尬了。
...
2. 给发起全局事务的入口方法添加@GloablTransactional注解
![](assets/分布式事务%20-%20Seata/file-20251128164802491.png)
...
阶段二回滚时RM的工作：
① 根据undo-log恢复数据到更新前
![](assets/分布式事务%20-%20Seata/file-20251128164802490.png)
**鱼和熊掌不可兼得**
